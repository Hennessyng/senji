...
log "Rebuilding gateway, readability, renderer"
docker compose up -d --build --force-recreate gateway readability renderer
...
for i in $(seq 1 24); do
    ...
    rn_ok=$(docker compose ps renderer    | grep -c '(healthy)' || true)
    if [ "$gw_ok" -ge 1 ] && [ "$rd_ok" -ge 1 ] && [ "$rn_ok" -ge 1 ]; then
        log "gateway, readability, renderer all healthy"
    ...
    if [ "$i" -eq 24 ]; then
        ...
        docker compose logs renderer    --tail=20 >> "$LOG"
        exit 1
    fi
    log "Waiting... gateway=${gw_ok}/1 readability=${rd_ok}/1 renderer=${rn_ok}/1 (${i}/24)"
    sleep 5
done
